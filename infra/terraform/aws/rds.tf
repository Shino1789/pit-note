...
resource "aws_db_instance" "main" {
  ...
  # db_nameは作成時のみ有効な属性であり、既存インスタンスに対しては
  # forces replacement（強制置き換え）となる。実機のdestroy→recoveryで
  # 検証済み: db_name込みでRDSが新規作成されることを確認しており、
  # 以後このインスタンスに対して値を変更するとreplacementが発生する
  db_name                     = "pitvia"
